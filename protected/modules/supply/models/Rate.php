...
{       
	...
	public function searchRates_supplier($get)
	{
		
		$criteria=new CDbCriteria;

		$criteria->select="LPAD(bundleid,8,0) as bundleid, 
                                    LPAD(t.rateid,8,0) as rateid,
                                    DATE(t.ratedate) as ratedate,
                                    rs.ratesupplierid,
                                    s.servicedsc,
                                    IF(t.quantity_1 IS NOT NULL,t.quantity_1,0) AS quantity_1,
                                    IF(t.quantity_2 IS NOT NULL,t.quantity_2,0) AS quantity_2,
                                    IF(t.quantity_3 IS NOT NULL,t.quantity_3,0) AS quantity_3,
                                    IF(t.quantity_4 IS NOT NULL,t.quantity_4,0) AS quantity_4,
                                    IF(t.quantity_5 IS NOT NULL,t.quantity_5,0) AS quantity_5,
                                    IF(t.quantity_6 IS NOT NULL,t.quantity_6,0) AS quantity_6,
                                    concat(eu.firstname, ' ', eu.plastname ) as comprador,
                                    concat(cc.name, ' ', cc.plastname, ' ', cc.mlastname ) as contacto, 
                                    br.branddsc as marca,
                                    pt.projectdsc,
                                    t.ratetype as tipo,
                                    t.note,
                                    t.version,
                                    t.statusid,
                                    rs.statusid as statussuplier,
                                    st.statusdsc as statusdsc,
                                    rs.statustime,
                                    LPAD(parentrateid,8,0) as parentrateid,
                                    IF(t.odptime IS NOT NULL, 1,0) AS odptime, 
                                    IF(t.odctime IS NOT NULL, 1,0) AS odctime
                                    ";
                
		 $criteria->join = "INNER JOIN project pt ON pt.projectid = t.projectid
                                    INNER JOIN brand br ON br.brandid = pt.brandid
                                    INNER JOIN service s ON s.serviceid = t.serviceid
                                    INNER JOIN employeeuser eu ON eu.userid = t.userid 
                                    INNER JOIN customercontact cc ON cc.contactid=t.customercontactid
                                    INNER JOIN ratesupplier rs on rs.rateid=t.rateid  and rs.supplierid = ".Yii::app()->user->supplierid."
                                    INNER JOIN status st ON st.statusid = t.statusid 
                                    INNER JOIN supplier spl on spl.supplierid=rs.supplierid
                                    ";
                                
                                $criteria->condition ="t.active = 1 AND rs.statusid=7 AND  DATE(t.ratedate) BETWEEN '".$get['start']."' AND '".$get['end']."' ";
                                    
                           
		//$criteria->group = 't.bundleid';	
		$criteria->order='t.bundleid desc';	
             //$criteria->select.' '. $criteria->join.' '.$criteria->condition;
		$cont=0;
		...
		{
                    $model= new RateController();
                    
                        $output[$cont]['bundleid'] = '<i class="ratepop" id="itemid_'.$row->rateid.'" data-tit="'.$row->servicedsc.'" data-content="'.$model->getDetail($row->rateid, $row->servicedsc, $row->note, $row->version).'" data-title="'.$row->servicedsc.'" data-placement="right" style="color:#0088CC; cursor:pointer;" data-original-title="" title="">'. $row->rateid.'</i>';
                        $output[$cont]['ratedate'] = $row->ratedate;
                        $output[$cont]['servicedsc'] = $row->servicedsc;
                        
                        if($row->quantity_1!= 0 )
                             $output[$cont]['quantity'] =$row->quantity_1;
                        if($row->quantity_2!= 0 )
                             $output[$cont]['quantity'] .=" ".$row->quantity_2;
                        if($row->quantity_3!= 0 )
                             $output[$cont]['quantity'] .=" ".$row->quantity_3;
                        if($row->quantity_4!= 0 )
                             $output[$cont]['quantity'] .=" ".$row->quantity_4;
                        if($row->quantity_5!= 0 )
                             $output[$cont]['quantity'] .=" ".$row->quantity_5;
                        if($row->quantity_6!= 0 )
                             $output[$cont]['quantity'] .=" ".$row->quantity_6;
                        
                        $output[$cont]['quantity1']= str_replace(' ','_',$output[$cont]['quantity']);  
                        
                        $output[$cont]['comprador'] = $row->comprador;
                        $output[$cont]['contacto'] = $row->contacto;
                        $output[$cont]['marca'] = $row->marca;
                        $output[$cont]['projectdsc'] = $row->projectdsc;
                        $output[$cont]['tipo'] = $row->tipo;
                        $output[$cont]['statusdsc'] = $row->statusdsc;
                        $concat=' ';
                        if($row->statussuplier==10 || $row->statusid==3  ){
                            $concat="disabled";
                        }
                         
                                if($row->statusid!=5 && $row->statussuplier!=10){
                                        $output[$cont]['accion'] = '<div class="btn-group"><button class="btn dropdown-toggle btn-warning" data-toggle="dropdown" '.$concat.'>Accion <i class="fa fa-caret-down"></i>'.
                                                   '</button><ul class="dropdown-menu pull-right">';    
                                    $output[$cont]['accion'] .='<li><a href="#newproject" id="resp_cot'.$row->rateid.'" data-target="#newproject" onclick="respon_cotizacion(this.id)" data-status="10"  data-price="'.$output[$cont]['quantity1'].'" data-id="'.$row->ratesupplierid.'" data-bundleid="'.$row->bundleid.'"  data-toggle="modal">Responder</a></li>'.
                                                    '<li><a href="#newproj" id="res_cot'.$row->rateid.'" data-toggle="modal" data-target="#newproj"  data-status="9" data-id="'.$row->ratesupplierid.'" onclick="respon_cot(this.id)" >Rechazar</a></li>';  //onclick="respon_cot(this.id)"
                                
                                }else{
                                    
                                    
                                    if($row->statussuplier==10){
                                        $output[$cont]['accion'] = '<div class="btn-group"><button class="btn dropdown-toggle btn-primary" data-toggle="dropdown" >Accion <i class="fa fa-caret-down"></i>'.
                                                   '</button><ul class="dropdown-menu pull-right">';
                                     $output[$cont]['accion'] .='<li><a href="#newproject" id="resp_cot'.$row->rateid.'" data-target="#newproject" onclick="respon_cotizacion(this.id)" data-status="10"  data-price="'.$output[$cont]['quantity1'].'" data-id="'.$row->ratesupplierid.'" data-bundleid="'.$row->bundleid.'"  data-toggle="modal">Editar</a></li>';
                                                    
                                        
                                     
                                    }else if ($row->statussuplier==11){
                                         $output[$cont]['accion'] = '<div class="btn-group"><button class="btn dropdown-toggle btn-success" data-toggle="dropdown" '.$concat.'>Accion <i class="fa fa-caret-down"></i>'.
                                                   '</button><ul class="dropdown-menu pull-right">';
                                         if($row->odctime != 0){                     
                                         $output[$cont]['accion'] .='<li><a href="" data-rate="'.$row->rateid.'" id="downodc" data-tipo="1" onclick="create_pdf(this.id)"  data-toggle="modal">Descargar ODC</a></li>';
                                            }
                                            if($row->odptime != 0){
                                                 $output[$cont]['accion'] .='<li><a href="" data-rate="'.$row->rateid.'" id="downodp" data-tipo="2" onclick="create_pdf(this.id)" data-toggle="modal">Descargar ODP</a></li>';
                                            }
                                    }
                                    
                                }
                                    $output[$cont]['accion'] .= '</ul></div>';
                       $cont=$cont+1;
		}
                }else{
                     $output = 0;
