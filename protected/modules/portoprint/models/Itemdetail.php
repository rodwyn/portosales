...
class Itemdetail extends CActiveRecord
{
	...
	{   
		...
		$criteria->select = " t.itemdetailid, t.itemdetaildsc  ";
	...
	public function getServiceDetail($serviceid, $conceptid)
	{   
		$criteria = new CDbCriteria;

		$criteria->select = " t.itemdetailid, t.itemdetaildsc, sd.servicedetailid  ";
		$criteria->join = " LEFT JOIN servicedetail sd ON sd.itemdetailid =  t.itemdetailid AND sd.serviceid = {$serviceid} AND sd.active = 1  ";
		$criteria->condition = " t.serviceid = {$conceptid} ";
		$criteria->order = "t.order";
		return $this->findAll($criteria);
