...
class Service extends CActiveRecord
{
	...
	public function getEntryItem($companyid, $userid)	{
		$criteria = new CDbCriteria ;
        ...
        $criteria->join = "inner join service s2 on s2.serviceid = t.serviceparentid and s2.active = 1
							...
							";
        $criteria->condition = " t.companyid = {$companyid} and us.userid = {$userid} and t.active = 1 and t.level = 4 ";
	...
	public function getSupplierService($supplierid){
        ...
        public function getServicesByCompany()
        {
            $companyid = Yii::app()->user->companyid;
            $criteria = new CDbCriteria;
            $criteria->select = "t.serviceid, t.companyid, t.servicedsc, t.serviceparentid, t.level, t.active ";
            $criteria->condition = "t.companyid = {$companyid} AND t.active = 1 AND t.serviceparentid = 0";
            return $this->findAll($criteria);
        }
       public function getServicesByCompanytree()
        {
            $companyid = Yii::app()->user->companyid;
            $criteria = new CDbCriteria;
            $criteria->select = "t.serviceid, t.companyid, t.servicedsc, t.serviceparentid, t.level, t.active ";
            $criteria->condition = "t.companyid = {$companyid} AND t.active = 1 AND t.level = 0 ";
            return $this->findAll($criteria);
        }
        
        public function getServicesByCompanytreesub($id)
        {
            $companyid = Yii::app()->user->companyid;
            $criteria = new CDbCriteria;
            $criteria->select = "t.serviceid, t.companyid, t.servicedsc, t.serviceparentid, t.level, t.active ";
            $criteria->condition = "t.companyid = {$companyid} AND t.active = 1 AND t.serviceparentid={$id}";
            return $this->findAll($criteria);
        }
        public function getSupplierService1($company){

		$criteria = new CDbCriteria ;
		$criteria->select = "t.serviceid, t.servicedsc ";
		$criteria->join = "inner join company c on c.companyid=t.companyid";
		$criteria->condition = "t.companyid={$company} and t.level=0 and t.active=1";
		return $this->findAll($criteria);
	}
        
         public function getSupplierService3($company,$id){

		$criteria = new CDbCriteria ;
		$criteria->select = "t.serviceid, t.servicedsc ";
		$criteria->join = "inner join company c on c.companyid=t.companyid"
                        . " inner join userservice us on us.serviceid=t.serviceid";
		$criteria->condition = "t.companyid={$company} and t.level=0 and t.active=1 and us.userid=".$id;
		return $this->findAll($criteria);
	}
        public function getSupplierService2($id){

		$criteria = new CDbCriteria ;
		$criteria->select = "us.serviceid, us.userserviceid";
		$criteria->join = "inner join userservice us on us.serviceid=t.serviceid";
		$criteria->condition = " t.level=0 and us.userid={$id} and t.active=1 and us.active=1";
		return $this->findAll($criteria);
	}
         public function getSupplierService4($company,$id,$serviceid){

		$criteria = new CDbCriteria ;
		$criteria->select = "t.serviceid, t.servicedsc ";
		
		$criteria->condition = "t.companyid={$company}  and t.serviceparentid={$serviceid} and t.active=1 ";
		return $this->findAll($criteria);
	}
