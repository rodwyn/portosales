...
class Usercustomerpermission extends CActiveRecord
{
	...
	public function getCustomerbyUser($userid, $companyid)	{
		$criteria = new CDbCriteria ;
        $criteria->select = " t.customerid, c.customerdsc  " ; 
        $criteria->join = "INNER JOIN customer c on c.customerid = t.customerid and c.companyid = {$companyid} and c.active = 1 ";
	...
	public function getuserservicebyCustomer($customerid, $entryid)	{
       ...
       public function validar_existencia($lista){
            ...
            $criteria=new CDbCriteria;
            $criteria->select = "t.usercustomerpermissionid";
            $cadena=" ";
            foreach ($lista as $valor => $descripcion){
                        if($cadena==" "){
                            $cadena=" t.".$valor." = ".  $descripcion;
       ...
       public function getCustomerUser($service,$user){

		$criteria = new CDbCriteria ;
		$criteria->select = "t.usercustomerpermissionid, t.active";
		$criteria->condition = " t.userid={$user} and t.customerid={$service}";
		return $this->findAll($criteria);
	}
}