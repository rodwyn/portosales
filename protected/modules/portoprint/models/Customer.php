...
class Customer extends CActiveRecord
{
	...
	public function relations()
	{
		...
		return array(
			'brands' => array(self::HAS_MANY, 'Brand', 'customerid'),
			...
			'downloadablereports' => array(self::HAS_MANY, 'Downloadablereport', 'customerid'),
			'usercustomerpermissions' => array(self::HAS_MANY, 'Usercustomerpermission', 'customerid'),
		);
	}
	...
	public function modelByUser($customerid,$userid)
	{
		$criteria = new CDbCriteria;
		$criteria->select = " t.* ";
		$criteria->join = "INNER JOIN usercustomerpermission ucp ON ucp.customerid = t.customerid";
		$criteria->condition = " t.customerid = {$customerid} AND ucp.userid = {$userid} and ucp.active = 1";
	...
	public function getAllCustomer()
	{
		...
		$criteria->select="t.customerid, t.customerdsc, t.email as email2";
                $criteria->condition = 't.companyid = '. Yii::app()->user->companyid;
        ...
        public function validar_campo($lista){
            ...
            foreach ($lista as $valor => $descripcion){
                        if($cadena==" " && $valor!="email"){
                            $cadena=" t.".$valor." = '".$descripcion."'";
                        }else if($valor!="email"){
                            $cadena=$cadena."and"." t.".$valor." = '".$descripcion."'";
                        }
                    }
            $condition = $cadena.' and t.companyid = '. Yii::app()->user->companyid;
