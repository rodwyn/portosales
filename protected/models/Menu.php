...
{       
	...
	public function search()
	{
        ...
        public function qryMenu($data,$userid) {
            ...
            $criteria->condition = "tm.usertype = {$data} AND active = 1 AND t.level=0";
            ...
            $menu="<ul>";
                        ...
                        foreach( $result as $row){
                            if($row->level==0){
                                ...
                                $menu .= '<li id="ml'.$ml.'" class="">
                                          <a href="'. Yii::app()->createUrl($row->menulink).'/read/'.$row->leer.'" title="'.$row->menudsc.'"><i class="'.$row->icon.'"></i> <span class="menu-item-parent">'.$row->menudsc.'</span></a>';
                                
                                $menu .= (count($smenudb)>0)?'<ul>':'';
                                foreach( $smenudb as $submenu){
                                    if($submenu->menuparentid==$row->menuid){
                                       
                                       
                                        $menu .= '<li >
                                                         <a href="'.Yii::app()->createUrl($submenu->menulink).'/read/'. $submenu->leer.'" title="'.$submenu->menudsc.'"><i class="'.$submenu->icon.'"></i> <span class="menu-item-parent">'.$submenu->menudsc.'</a>
                                                    </li>';
                                    }
                                }
                                $menu .= (count($smenudb)>0)?'</ul>':'';
